...
import { dealCellClear, onCellClear, onCellSelect, onRCShowHide } from "../customs/method";
import { renderRCHiddenMarks } from '../customs/markMethod';
...
export function rowColumnOperationInitial() {
    ...
    //隐藏、显示行
    $('#luckysheet-hide-selected').click(function () {
        if ("column" === Store.luckysheetRightHeadClickIs) { // 隐藏列
            $("#luckysheet-hidCols").trigger('click');
        } else {
            $("#luckysheet-hidRows").trigger('click');
        }
    })
    $('#luckysheet-show-selected').click(function () {
        if ("column" === Store.luckysheetRightHeadClickIs) { // 隐藏列
            $("#luckysheet-showHidCols").trigger('click');
        } else {
            $("#luckysheet-showHidRows").trigger('click');
        }
    })
    ...
    function showHideCol(isShow) {
        ...
        for (let s = 0, len = Store.luckysheet_select_save.length; s < len; s++) {
        ...
        // 【自改】回调自定义函数
        onRCShowHide('c', inds, isShow);
        renderRCHiddenMarks();
    }
    function showHideRow(isShow) {
        // 【自改】
        let inds = [];
