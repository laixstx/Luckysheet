...
import { onRCAdd, onRCDelete } from '../customs/method';
...
function jfrefreshgrid_adRC(data, cfg, ctrlType, ctrlValue, calc, filterObj, cf, af, freezen) {
    ...
    let index = ctrlValue.index, // 操作的行/列索引
        len = ctrlValue.len, // 操作的行/列数量
        rc = ctrlValue.rc; // 操作的类型。r: 行。c: 列
    ...
    if (ctrlType == "addRC") {
        let direction = ctrlValue.direction;
        onRCAdd(rc, index, len, direction);
    } else if (ctrlType == 'delRC') {
        onRCDelete(rc, index, len);
    }
}
